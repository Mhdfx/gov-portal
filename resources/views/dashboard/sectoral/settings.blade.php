...
<nav class="flex-1 px-2 py-4 space-y-1">
    ...
    <a href="{{ route('sectoral.profile') }}" class="group flex items-center px-3 py-2 text-sm font-medium rounded-lg text-gray-700 hover:bg-gray-100">
        <i class="ri-user-line text-xl mr-3"></i>
        Profile
    </a>
    <a href="{{ route('sectoral.settings') }}" class="group flex items-center px-3 py-2 text-sm font-medium rounded-lg bg-blue-50 text-blue-700">
...
<div class="px-4 sm:px-6 lg:px-8">
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <p class="text-sm uppercase tracking-wide text-gray-500">Sectoral Admin • Configuration</p>
            <h1 class="text-3xl font-bold text-gray-900">Settings</h1>
            <p class="mt-2 text-gray-600">Configure sectoral preferences and notification settings.</p>
        </div>
        @if(session('success'))
    ...
    <form method="POST" action="{{ route('sectoral.settings.update') }}" class="space-y-8">
        @csrf

        <!-- Sector Information -->
        <div class="bg-white shadow rounded-2xl p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-6 flex items-center">
                <i class="ri-folder-chart-line text-2xl mr-3 text-blue-600"></i>
                Sector Information
            </h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Sector Name *</label>
                    <input type="text" name="sector_name" value="{{ old('sector_name', $settings['sector_name']) }}" required
                        class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                    @error('sector_name')<p class="text-xs text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
        ...
        <div class="bg-white shadow rounded-2xl p-6">
            ...
            <div class="space-y-4">
                ...
                <div class="flex items-center">
                    ...
                    <p class="text-xs text-gray-500 ml-2">Only verified users can submit to your sector.</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end gap-3">
            <a href="{{ route('sectoral.dashboard') }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">
                Cancel
            </a>
