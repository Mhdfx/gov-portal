...
<nav class="flex-1 px-2 py-4 space-y-1">
    ...
    <a href="{{ route('admin.users') }}" class="group flex items-center px-3 py-2 text-sm font-medium rounded-lg bg-blue-50 text-blue-700">
        <i class="ri-user-line text-xl mr-3"></i>
        Users
        <span class="ml-auto bg-blue-100 text-blue-700 px-2 py-0.5 text-xs rounded-full">{{ $stats['total'] }}</span>
    </a>
...
<div class="px-4 sm:px-6 lg:px-8 space-y-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <p class="text-sm uppercase tracking-wide text-gray-500">Administration • Accounts</p>
            <h1 class="text-3xl font-bold text-gray-900">Users Management</h1>
            <p class="mt-2 text-gray-600">Track every persona on the platform, supervise verifications, and act quickly on blockers.</p>
        </div>
        <div class="flex flex-wrap gap-3">
            ...
            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600">
                Showing {{ $users->total() }} / {{ number_format($stats['total']) }} users
            </span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white shadow rounded-2xl p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm text-gray-500">Company accounts</p>
                    <p class="text-2xl font-semibold text-gray-900">{{ number_format($stats['companies']) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white shadow rounded-2xl p-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1" for="status">Verification</label>
            ...
            <div>
                ...
                <select id="sort" name="sort" class="w-full rounded-lg border-gray-300 focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <option value="status" @selected($filters['sort'] === 'status')>Verification status</option>
                </select>
            </div>
            <div class="md:col-span-2">
            ...
            <div class="md:col-span-3 flex items-end gap-3">
                ...
                <a href="{{ route('admin.users') }}" class="inline-flex items-center px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50">
                    Reset
                </a>
            </div>
        </form>
    ...
    <!-- Role breakdown -->
    <div class="bg-white shadow rounded-2xl p-6">
        <h3 class="text-sm font-semibold text-gray-700 uppercase tracking-wide mb-4">Role distribution</h3>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            @foreach($roles as $roleKey => $label)
                <div class="flex flex-col border border-gray-100 rounded-xl p-4">
                    <span class="text-xs uppercase text-gray-500">{{ $label }}</span>
                    <span class="text-2xl font-semibold text-gray-900">{{ $roleBreakdown[$roleKey] ?? 0 }}</span>
                </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="bg-white shadow rounded-2xl">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 text-sm">
                        ...
                        @foreach($users as $user)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div>
                                            ...
                                            <p class="text-xs text-gray-500">{{ optional($user->profile)->full_name }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <p class="text-sm text-gray-900">{{ $user->email }}</p>
                                    <p class="text-xs text-gray-500">{{ optional($user->profile)->phone }}</p>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold {{ $statusColors[$user->verification_status] ?? 'bg-gray-100 text-gray-700' }}">
                                        {{ ucfirst(str_replace('_', ' ', $user->verification_status)) }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-gray-700">
                                    ...
                                    <p class="text-xs text-gray-500">Last login: {{ $user->last_login_at ? $user->last_login_at->diffForHumans() : '—' }}</p>
                                </td>
        ...
        @else
            <div class="p-12 text-center">
                ...
                <a href="{{ route('admin.users') }}" class="inline-flex items-center px-4 py-2 rounded-lg bg-blue-600 text-white font-medium shadow hover:bg-blue-700">
                    Reset filters
                </a>
            </div>
        @endif
